...
# 預處理地圖（對齊原點 + 解析度）
# =========================
def preprocess_map(img, offset_x, offset_y, resolution):
    # 做平移處理（對齊原點）
    rows, cols = img.shape
    translation_matrix = np.float32([[1, 0, offset_x / resolution], [0, 1, offset_y / resolution]])

    # 做仿射轉換（平移）
    img_aligned = cv2.warpAffine(img, translation_matrix, (cols, rows))
...
if __name__ == "__main__":
    ...
    # =========================
    # 預處理地圖（將SLAM map對齊GT map原點）
    # =========================
    slam_aligned = preprocess_map(slam_img, offset_x, offset_y, resolution=0.05)
    ...
    # save aligned result
    cv2.imwrite(BASE_PATH + "/aligned_slam.pgm", slam_aligned)

    print("Saved: aligned_slam.pgm")